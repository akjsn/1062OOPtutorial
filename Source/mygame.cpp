...
void CGameStateRun::OnMove()							// 移動遊戲元素
{
	...
	// 移動擦子
	//
	//practice.SetTopLeft(10, 10); //disabled in 3
	if (picX <= SIZE_Y)			   //replacing above code in 3
	{
		picX += 5;
		picY += 5;
	}
...
void CGameStateRun::OnInit()  								// 遊戲的初值及圖形設定
{
	...
	ShowInitProgress(50);
	Sleep(300); // 放慢，以便看清楚進度，實際遊戲請刪除此Sleep
	practice.LoadBitmap(IDB_APPLE);
	practice2.LoadBitmap("Bitmaps/triangle.bmp");
	practice3.LoadBitmap("Bitmaps/border.bmp", RGB(255, 255, 255));
...
{
	pic.LoadBitmap("Bitmaps/potato.bmp");
}
...
void CGameMap::InitializeBouncingBall(int ini_index, int row, int col)
{
	...
	const int ballpicheight = 15;
	int floor = Y + (row + 1)*MH - ballpicheight;
